    ...
    <>
      ...
      <aside className="hidden lg:flex flex-col w-56 shrink-0 h-full bg-white border-r border-[#F0EDE8]">
        ...
        <div className="px-4 py-4 border-t border-[#F0EDE8] flex items-center gap-3">
          ...
          <span className="text-sm text-[#5A5A5A] font-medium flex-1 truncate">
            {displayName}
          </span>
          <form action={logout}>
            <button
              type="submit"
              title="Log out"
              className="text-[#ADADAD] hover:text-[#5A5A5A] transition-colors p-1 rounded-lg hover:bg-[#F5F3EF]"
            >
          ...
          return (
            <Link
              ...
              className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-[10px] transition-colors ${
                active ? "text-[#7C9082] font-medium" : "text-[#ADADAD]"
              }`}
            >
